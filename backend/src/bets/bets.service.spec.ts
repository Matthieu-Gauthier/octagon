...
describe('BetsService', () => {
  ...
  // Helpers
  // -------------------------------------------------------------------------
  const future = (offsetMs = 24 * 60 * 60 * 1000) =>
    new Date(Date.now() + offsetMs);
  const past = (offsetMs = 24 * 60 * 60 * 1000) =>
    new Date(Date.now() - offsetMs);
  ...
  describe('placeBet - fight not found', () => {
    it('should throw NotFoundException if fight does not exist', async () => {
      ...
      await expect(
        service.placeBet(userId, leagueId, fightId, betData),
  ...
  describe('placeBet - prelim fights (US1)', () => {
    it('T012: should accept bet on prelim fight when now < prelimsStartAt', async () => {
      ...
      (prisma.bet.upsert as jest.Mock).mockResolvedValue({
        id: 'bet-1',
        ...betData,
      });
