...
export class ScraperService {
  ...
  async scrapeEventBySlug(eventSlug: string): Promise<ScrapedEventData | null> {
    try {
      ...
      const eventName =
        $ev('.c-hero__headline-prefix').text().trim() +
        ' ' +
        $ev('.c-hero__headline').text().trim();
      const timestamp =
        $ev('.c-hero__headline-suffix').attr('data-timestamp') ||
        $ev('.c-hero__headline-suffix').attr('data-datetime');
      const eventDate = timestamp
        ? new Date(parseInt(timestamp) * 1000)
        : new Date();
      const location =
        $ev('.c-hero__headline-suffix').text().trim() || 'Unknown';
      ...
      const event: ScrapedEvent = {
        ...
        name:
          (eventName || eventSlug?.replace(/-/g, ' ').toUpperCase()) ??
          'Unknown',
        date: eventDate,
        location,
        status: 'SCHEDULED',
      ...
      for (const section of sections) {
        ...
        for (let i = 0; i < fightElements.length; i++) {
          ...
          fights.push({
            ...
            order: globalIndex,
            rounds:
              globalIndex === 0 || weightClass.toLowerCase().includes('title')
                ? 5
                : 3,
   ...
   */
  private extractSectionTimestamp(
    $: ReturnType<typeof cheerio.load>,
    sectionId: string,
  ): Date | undefined {
    const ts = $(`${sectionId} .c-event-fight-card-broadcaster__time`)
      .first()
      .attr('data-timestamp');
  ...
  private async scrapeFighter(slug: string): Promise<ScrapedFighter> {
    ...
    const sigStrikesLandedPerMin =
      parseFloat(
        $('.c-stat-compare')
          .first()
          .find('.c-stat-compare__number')
          .first()
          .text()
          .trim(),
      ) || null;
