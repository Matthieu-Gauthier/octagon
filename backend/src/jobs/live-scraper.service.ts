...
export class LiveScraperService {
    ...
    async handleLiveEvents() {
        ...
        try {
            ...
            this.logger.error(`Error during live event scraping: ${error.message}`, error.stack);
        }
    }

    private async scrapeLiveEvent(event: any) {
        ...
        try {
            ...
            for (const fight of event.fights) {
                ...
                $('.c-listing-fight').each((_, el) => {
                    ...
                    const fASlug = $(fighterLinks[0]).attr('href')?.split('/').pop() || '';
                    const fBSlug = $(fighterLinks[1]).attr('href')?.split('/').pop() || '';

                    if ((fASlug === fight.fighterAId && fBSlug === fight.fighterBId) ||
                        (fASlug === fight.fighterBId && fBSlug === fight.fighterAId)) {

                        fightFound = true;

                        // Check winner using .c-listing-fight__outcome--win
                        const isRedWinner = $(el).find('.c-listing-fight__corner-body--red .c-listing-fight__outcome--win').length > 0;
                        const isBlueWinner = $(el).find('.c-listing-fight__corner-body--blue .c-listing-fight__outcome--win').length > 0;
                        // Check No Contest using .c-listing-fight__outcome--no-contest
                        const isNoContest = $(el).find('.c-listing-fight__outcome--no-contest').length > 0;
                        ...
                        const methodText = $(el).find('.c-listing-fight__result-text.method').text().trim().toLowerCase();
                        ...
                        if (methodText) {
                            ...
                            if (methodText.includes('ko') || methodText.includes('tko')) {
                                ...
                                newMethod = 'DRAW';
                            } else if (methodText.includes('nc') || methodText.includes('no contest')) {
                                newMethod = 'NC';
                            } else {
                        ...
                        const roundText = $(el).find('.c-listing-fight__result-text.round').text().trim();
                        if (roundText) {
                        ...
                        if (isFinished && !newMethod) {
                            const backupText = $(el).find('.c-listing-fight__result-text').text().trim().toLowerCase();
                            if (backupText.includes('ko') || backupText.includes('tko')) {
                                ...
                                newMethod = 'DRAW';
                            } else if (backupText.includes('nc') || backupText.includes('no contest')) {
                                newMethod = 'NC';
                            }
                        }

                        if (isFinished && !newRound) {
                            const roundMatch = $(el).text().match(/round[\s:]*(\d+)/i) || $(el).text().match(/r(\d+)/i);
                            if (roundMatch) {
            ...
            this.logger.error(`Failed to scrape live event ${event.id}: ${err.message}`);
        }
    }
