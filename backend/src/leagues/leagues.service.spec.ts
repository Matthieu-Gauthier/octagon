...
describe('LeaguesService', () => {
    ...
    describe('join', () => {
        it('should add a member to the league if code is valid', async () => {
            ...
            (prisma.leagueMember.create as jest.Mock).mockResolvedValue({ leagueId: 'league-1', userId });
        ...
        it('should throw NotFoundException if code is invalid', async () => {
            ...
            await expect(service.join('INVALID', 'user-2')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw BadRequestException if already a member', async () => {
            ...
            await expect(service.join(code, 'user-2')).rejects.toThrow(BadRequestException);
        });
    });
});
